...
class DailyScanner:
    ...
    def money_money_value_with_15min_data(self, symbolToken, clientCode):
        ...
        if(clientCode == 1):
            first_16_min = self.historicalClient.get_candle_data(symbolToken, CandleIntervals.ONE_MINUTE, today_date_9_15, today_date_9_30)
        elif(clientCode == 2):
            first_16_min = self.historicalClient2.get_candle_data(symbolToken, CandleIntervals.ONE_MINUTE, today_date_9_15, today_date_9_30)
        elif(clientCode == 3):
            first_16_min = self.historicalClient3.get_candle_data(symbolToken, CandleIntervals.ONE_MINUTE, today_date_9_15, today_date_9_30)

        percentage_change = self.get_percentage_change2(symbolToken, first_16_min)
        money_money_factor = self.get_mm_factor2(symbolToken, first_16_min)
        # self.historicalClient.log_out()
        return ((percentage_change * money_money_factor),first_16_min)
